...
#
# Result as of 2026-08-11 (see also 96 for the healthy-donor arm):
#   sensitivity on genotyped mut+ cells .................. 0.484
...
calls <- rbindlist(lapply(unique(geno$sample), function(s) {
  ...
  d <- fread(f, select = c("cell", "malignant"))
  d[, sample := s][]
}), fill = TRUE)
...
m <- merge(geno, calls[, .(k, malignant)], by = "k")
