...
The pipeline runs end-to-end and its statistical machinery is, if anything, unusually self-critical (permutation nulls, FWL residualization, a pre-registered alpha sweep, a platform control, and feature-decomposition diagnostics). But **the three headline hypotheses do not hold, and they fail along a single causal chain that starts at the label layer:**

> Single-arm inferCNV labelling → **~40% false-positive rate in the HSC_MPP (LSC) bin** → the `frac_malignant` node feature is both unreliable and, for healthy samples, **forced to 0 by construction** → the only statistically significant term behind H2 is that circular feature plus raw cell count → **at pure topology (α=1) H2 is null** → with no topological signal, **no communication edge survives permutation (H1 null)** → H3's only nominal signal is a single-patient, label-dependent artifact on metadata that is itself unverified (**H3 blocked**).

**The one solid, publishable-as-is result is the H1 null** — because H1's edges come straight from CellChat and never pass through the malignancy labels, so it is immune to the label-quality problem. It is a genuine negative, not a borderline miss.
...
| **H2** (topology carries information) | 🔴 **not topological** | α=1 (pure topology) null (p=0.105/0.271); signal = circular `frac_malignant` + cell count |
| **H3** (treatment-pressure axis) | ⚫ **blocked** | Only a V-shape (not monotone); 62% of MRD malignant cells from one patient; timepoint/pairing metadata unverified |
...
**Because H1's edges come straight from CellChat and never touch the malignancy labels, this null is immune to the §1 label-quality problem. It is the most solid result the project currently has.**

### H2 — topology carries disease information — 🔴 not topological *(depends on labels)*

The decisive pre-registered test is the alpha sweep. At the **pure-topology endpoint α=1**: p_global=0.105, p_strat=0.271, beta_global=0.012 — **null.** The signal lives entirely at the feature end (α=0: p_global=0.0009). Decomposing the features:
...
So whatever discrimination exists is blast fraction (circular) + cell count (technical), **not graph topology, and not stemness.** Note the earlier H2-supporting scripts (`01_h2_blast_regression`, `05_h2_platform_control`, dated Jul 21–22) report H2 as *supported*, but they are fixed at α=0.5 and predate the Jul 24 diagnostics that reattribute the signal — their "support" is exactly the composition artifact the α=1 test exposes.

### H3 — treatment-pressure axis — ⚫ blocked *(depends on labels + unverified metadata)*

The pre-registered three-model (A/B/C) competition cannot be adjudicated — no A/B/C fit table exists. What exists is not monotone: stem_frac and primitive_frac fall Dx→MRD (both p=0.031, n=6, all 6 pairs down) but rebound MRD→Relapse (p=0.063, n.s.) and net Dx→Relapse is null (p=0.156 / 0.688) — a **V-shape, not the predicted monotone deepening**, and only 2 of 4 stemness signatures even show the V (LSC17 and vanGalen_HSC_Prog do; HSPC_core is monotone-down, vanGalen_HSC_like peaks at MRD).

**Decisively, the axis is patient-confounded and label-dependent:** 62% of all MRD malignant cells (5324 of ~8634) come from a single sample, 6323_MRD, which is 98.7% Mono_DC with primitive_frac=0.006 — so the MRD stem-collapse is essentially one patient whose malignant compartment was labelled almost entirely non-primitive. The comparison rests almost entirely on GSE227903 (the only dataset with clean Dx/MRD/Relapse triplets — 23 of 130 samples carry any timepoint; the other 107 are cross-sectional/baseline by design, not broken metadata). Timepoints in other datasets are unverified against the source papers. **H3 is untestable as specified until the metadata reconciliation is done.**
...
**Sequencing (already agreed):** fix metadata (timepoint/pairing in the affected datasets) → add the CopyKAT/SCEVAN arm → re-run `50_consensus` (which invalidates 03–08) → re-run downstream **once**, folding in the GSE289435 Numbat merge at the same time. Only after that does it make sense to add the missing pieces the blueprint specified but the pipeline skipped: the SRRS reproducibility screen (Phase 3), the R3 two-method CCC gate (Phase 4), M8 feature discipline (Phase 5–6), and the entire Phase 8 robustness stage.
